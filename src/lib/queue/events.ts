...
export const setupQueueEvents = () => {
  const reviewEvents = new QueueEvents(QUEUE_NAMES.REVIEW, { connection: redisConnection });
  const agentEvents = new QueueEvents(QUEUE_NAMES.AGENT, { connection: redisConnection });
